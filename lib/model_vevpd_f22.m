function [stress,F_tot,F_ve_tot,F_v_tot,F_e_tot,F_vp_tot,J_tot,timeVec,...
    ...
    strain_gl,tau_tot,dt,C_all] = model_calibr(params,d0,trueStrain,vp,params_F,nnetwork,...
    cyclic_loading,et_load,et_unload,type,load_type,perturbation)
format longG
global eps0
eps0 = 0;
...
for j = 1:length(trueStrain)
        ...
        F = [F22 0 0; 0 F11 0; 0 0 F22]; % Incremental F at t+1
        J  = det(F);
        Fb = J^(-1/3) * F;
        Cb = Fb'*Fb;
        E_gl = 1/2*(Cb-eye(3));
